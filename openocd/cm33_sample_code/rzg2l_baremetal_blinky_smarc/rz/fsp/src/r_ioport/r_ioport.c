...
#define IOPORT_PRV_BITFIELD_1BIT          (1UL)
#define IOPORT_PRV_BITFIELD_2BIT          (3UL)
#define IOPORT_PRV_BITFIELD_3BIT          (7UL)

#define IOPORT_PRV_SET_PMC                (1UL)
...
static void r_ioport_pin_config_peri_mode (bsp_io_port_pin_t pin, uint32_t value);
static void r_ioport_pin_config_port_mode (bsp_io_port_pin_t pin, uint32_t value);
static void make_regwrite_value (uint8_t shift, uint8_t mask, uint32_t *value);
static void make_mask_8 (uint8_t pin, uint8_t bitfield, uint8_t *p_mask);
static void make_mask_16 (uint8_t pin, uint16_t bitfield, uint16_t *p_mask);
static void make_mask_32 (uint8_t pin, uint32_t bitfield, uint32_t *p_mask);
static void make_mask_64 (uint8_t pin, uint64_t bitfield, uint64_t *p_mask);
static void make_port_pin_num (bsp_io_port_pin_t pin, uint8_t *p_port_number, uint8_t *p_pin_number);
static void RZA_IO_RegWrite_8(volatile uint8_t * ioreg, uint8_t write_value, uint8_t shift, uint32_t mask);
static void RZA_IO_RegWrite_16(volatile uint16_t * ioreg, uint16_t write_value, uint16_t shift, uint32_t mask);
static void RZA_IO_RegWrite_32(volatile uint32_t * ioreg, uint32_t write_value, uint32_t shift, uint32_t mask);
static void RZA_IO_RegWrite_64(volatile uint64_t * ioreg, uint64_t write_value, uint64_t shift, uint64_t mask);
...
void r_ioport_pins_config (const ioport_cfg_t * p_cfg)
{
    ...
    /* Originally, it is necessary processing of Enable/Disable PFS Register writeprotection each access of PFS Register,
     * but from the viewpoint of reusing RA FSP code, processing of Enable/Disable PFS Register writeprotection are written here. */
...
static void r_ioport_pfs_write (bsp_io_port_pin_t pin, uint32_t value)
{
    ...
    {
    	r_ioport_pin_config_peri_mode(pin, value);
    }
    else
    {
    	r_ioport_pin_config_port_mode(pin, value);
    }
}
...
static void r_ioport_pin_config_peri_mode (bsp_io_port_pin_t pin, uint32_t value)
{
    ...
    uint8_t mask_pmc;
    uint32_t mask_pfc;
    uint32_t value_pfc = value;

    volatile uint64_t *p_iolh;
    volatile uint64_t *p_sr;
    volatile uint64_t *p_pupd;
    uint64_t mask_iolh;
    uint64_t mask_sr;
    uint64_t mask_pupd;
    uint32_t value_iolh = value;
    uint32_t value_sr   = value;
    uint32_t value_pupd = value;

	p_pmc = &R_PORT->PMC[0].BYTE;
	p_pfc = &R_PORT->PFC[0].DWORD;
	make_port_pin_num(pin, &u_port, &u_pin);
	make_mask_8(u_pin, IOPORT_PRV_BITFIELD_1BIT, &mask_pmc);
	make_mask_32(u_pin, IOPORT_PRV_BITFIELD_3BIT, &mask_pfc);
	make_regwrite_value(IOPORT_PRV_PFC_OFFSET, IOPORT_PRV_BITFIELD_3BIT, &value_pfc);

	RZA_IO_RegWrite_8(&p_pmc[u_port], IOPORT_PRV_SET_PMC, u_pin, mask_pmc);

	/* Refer comment of r_ioport_pins_config() about why R_BSP_PinAccessEnable() and R_BSP_PinAccessDisable() are commentout. */
    //R_BSP_PinAccessEnable();           // Protect PWPR from re-entrancy

	RZA_IO_RegWrite_32(&p_pfc[u_port], value_pfc, (uint32_t) u_pin, mask_pfc);

    //R_BSP_PinAccessDisable();

    /* Config of IOLH Register, SR Register, PUPD Register */
	p_iolh = &R_PORT->IOLH[0].QWORD;
	p_sr = &R_PORT->SR[0].QWORD;
	p_pupd = &R_PORT->PUPD[0].QWORD;
	make_port_pin_num(pin, &u_port, &u_pin);
	make_mask_64(u_pin, IOPORT_PRV_BITFIELD_2BIT, &mask_iolh);
	make_mask_64(u_pin, IOPORT_PRV_BITFIELD_1BIT, &mask_sr);
	make_mask_64(u_pin, IOPORT_PRV_BITFIELD_2BIT, &mask_pupd);
	make_regwrite_value(IOPORT_PRV_IOLH_OFFSET, IOPORT_PRV_BITFIELD_2BIT, &value_iolh);
	make_regwrite_value(IOPORT_PRV_SR_OFFSET, IOPORT_PRV_BITFIELD_1BIT, &value_sr);
	make_regwrite_value(IOPORT_PRV_PUPD_OFFSET, IOPORT_PRV_BITFIELD_2BIT, &value_pupd);

	RZA_IO_RegWrite_64(&p_iolh[u_port], (uint64_t) value_iolh, (uint64_t) u_pin, mask_iolh);
	RZA_IO_RegWrite_64(&p_sr[u_port], (uint64_t) value_sr, (uint64_t) u_pin, mask_sr);
	RZA_IO_RegWrite_64(&p_pupd[u_port], (uint64_t) value_pupd, (uint64_t) u_pin, mask_pupd);
}
...
static void r_ioport_pin_config_port_mode (bsp_io_port_pin_t pin, uint32_t value)
{
    ...
    uint16_t mask_pm;
    uint8_t mask_p;
    uint32_t value_pm = value;
    uint32_t value_p = value;

	p_pm = &R_PORT->PM[0].WORD;
	p_p = &R_PORT->P[0].BYTE;
	make_port_pin_num(pin, &u_port, &u_pin);
	make_mask_16(u_pin, IOPORT_PRV_BITFIELD_2BIT, &mask_pm);
	make_mask_8(u_pin, IOPORT_PRV_BITFIELD_1BIT, &mask_p);
	make_regwrite_value(IOPORT_PRV_PM_OFFSET, IOPORT_PRV_BITFIELD_2BIT, &value_pm);
	make_regwrite_value(IOPORT_PRV_P_OFFSET, IOPORT_PRV_BITFIELD_1BIT, &value_p);

    /* Write configuration */
	RZA_IO_RegWrite_16(&p_pm[u_port], (uint16_t) value_pm, u_pin, mask_pm);
	RZA_IO_RegWrite_8(&p_p[u_port], (uint8_t) value_p, u_pin, mask_p);
}
...
static void make_mask_8 (uint8_t pin, uint8_t bitfield, uint8_t *p_mask)
{
	*p_mask = (uint8_t) (bitfield << pin);
}

static void make_mask_16 (uint8_t pin, uint16_t bitfield, uint16_t *p_mask)
{
	*p_mask = (uint16_t) (bitfield << (pin * 2));
}

static void make_mask_32 (uint8_t pin, uint32_t bitfield, uint32_t *p_mask)
{
	*p_mask = (uint32_t) (bitfield << (pin * 4));
}

static void make_mask_64 (uint8_t pin, uint64_t bitfield, uint64_t *p_mask)
{
	*p_mask = (uint64_t) (bitfield << (pin * 8));
}
...
static void make_port_pin_num (bsp_io_port_pin_t pin, uint8_t *p_port_number, uint8_t *p_pin_number)
{
    ...
    *p_port_number = (uint8_t) (pin >> IOPORT_PRV_PORT_OFFSET);

    /* It is okay for using lower 8bit of unsigned 16bit pin value */
    *p_pin_number = (uint8_t) (pin & BSP_IO_PRV_8BIT_MASK);
}
/******************************************************************************
...
static void RZA_IO_RegWrite_16(volatile uint16_t * ioreg, uint16_t write_value, uint16_t shift, uint32_t mask)
{
    ...
    if (IOREG_NONMASK_ACCESS != mask)
    {
        ...
        /* Modify value */
        reg_value = (uint16_t)((reg_value & (~mask)) | (uint16_t)(write_value << (shift * 2)));
    }
    else
    {
        reg_value = write_value;
...
static void RZA_IO_RegWrite_32(volatile uint32_t * ioreg, uint32_t write_value, uint32_t shift, uint32_t mask)
{
    ...
    if (IOREG_NONMASK_ACCESS != mask)
    {
        ...
        /* Modify value */
        reg_value = (uint32_t)((reg_value & (~mask)) | (uint32_t)(write_value << (shift * 4)));
    }
    else
    {
        reg_value = write_value;
...
/**********************************************************************************************************************
 * Function Name: RZA_IO_RegWrite_64
 * Description  : IO register 64-bit write
 * Arguments    : volatile uint64_t * ioreg : IO register for writing
 *              :                           : Use register definition name of the iodefine.h
 *              : uint64_t write_value      : Write value for the IO register
 *              : uint64_t shift            : The number of left shifts to the target bit
 *              : uint64_t mask             : Mask value for the IO register (Target bit : "1")
 * Return Value : None
 *********************************************************************************************************************/
static void RZA_IO_RegWrite_64(volatile uint64_t * ioreg, uint64_t write_value, uint64_t shift, uint64_t mask)
{
    ...
    if (IOREG_NONMASK_ACCESS != mask)
    {
        ...
        /* Modify value */
        reg_value = (uint64_t)((reg_value & (~mask)) | (uint64_t)(write_value << (shift * 8)));
    }
    else
    {
        reg_value = write_value;
...
/**********************************************************************************************************************
 * End of function RZA_IO_RegWrite_64
/**********************************************************************************************************************
 ...
 *********************************************************************************************************************/
static uint8_t RZA_IO_RegRead_8(volatile uint8_t * ioreg, uint8_t shift, uint32_t mask)
{
    uint8_t reg_value;
 ...
 *********************************************************************************************************************/
static uint16_t RZA_IO_RegRead_16(volatile uint16_t * ioreg, uint16_t shift, uint32_t mask)
{
    uint16_t reg_value;
 ...
 *********************************************************************************************************************/
static uint32_t RZA_IO_RegRead_32(volatile uint32_t * ioreg, uint32_t shift, uint32_t mask)
{
    uint32_t reg_value;
