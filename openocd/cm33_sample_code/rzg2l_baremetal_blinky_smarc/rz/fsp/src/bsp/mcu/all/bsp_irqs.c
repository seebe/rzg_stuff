 ...
 * Macro definitions
 **********************************************************************************************************************/
#define BSP_CM33_CPG_SETTING     (1)            /* Set 0 when set CPG in CA55. Set 1 when set CPG in CM33.*/
#define BSP_TEMPORARY_DISABLE    (1)            /* Decide whether to enable after an investigation(20210406).*/
...
{
#if BSP_CM33_CPG_SETTING == 1
    #if BSP_TEMPORARY_DISABLE == 1
    R_BSP_CLKON(BSP_CLK_DMAC_REG,0);
    R_BSP_RSTOFF(BSP_CLK_DMAC_REG,0);
    #endif
    R_BSP_CLKON(BSP_CLK_DMAC_REG,1);
    R_BSP_RSTOFF(BSP_CLK_DMAC_REG,1);
#endif
    R_BSP_IM33_ENABLE();
